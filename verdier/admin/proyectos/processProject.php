...
function addProject()
{
	...
	$image = uploadProductImage('fleImage1', SRV_ROOT . 'images/Planos/');
	
	$descripcion = $_POST['txtDesc'];
	
	$sql   = "INSERT INTO `proyecto` (`id`, `nombre_imagen`) VALUES (NULL, '$image')";
		ejecutarSql($sql);
		header('Location: index.php');	
...
function _deleteImage($id)
{
	...
	$imagen = devolverImagenProyecto($id);

	// remove the image file
	
	@unlink(SRV_ROOT . 'Planos/' . $imagen);
	@unlink(SRV_ROOT . 'Planos/' ."thumbnail_". $imagen);
	@unlink(SRV_ROOT . 'Planos/' ."resized_". $imagen);		
	header('Location: index.php');
