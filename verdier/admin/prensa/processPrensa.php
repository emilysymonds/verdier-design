...
function addPrensa()
{
	...
	$image = uploadProductImage('fleImage1', SRV_ROOT . 'images/Prensa/');
	
	$descripcion = $_POST['txtDesc'];
	
	$sql   = "INSERT INTO `prensa` (`id`, `nombre_imagen`) VALUES (NULL, '$image')";
		ejecutarSql($sql);
		header('Location: index.php');	
...
function _deleteImage($id)
{
	...
	$imagen = devolverImagenPrensa($id);

	// remove the image file
	
	@unlink(SRV_ROOT . 'Prensa/' . $imagen);
	@unlink(SRV_ROOT . 'Prensa/' ."thumbnail_". $imagen);
	@unlink(SRV_ROOT . 'Prensa/' ."resized_". $imagen);		
	header('Location: index.php');
